...
public class DBMSBound {
    ...
    public void insertComunicazione(int idComunicazione, int mittente, int destinatario, LocalDate oggi, String tipo_comunicazione){
        try {
            Class.forName("com.mysql.jdbc.Driver");
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3308/azienda", "root", "");
            stm = connection.createStatement();
    ...
    public ResultSet acquisisciListaComuniazioni(int matricola){
        try {
            stm = connection.createStatement();
            rs = stm.executeQuery("SELECT * FROM comunicazioni  WHERE ref_utente_destinatario = " + matricola + " AND ref_utente_mittente != " + matricola + " ORDER BY flag_visualizzazione ASC, data_comunicazione DESC");
            return rs;
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    public int acquisisciOreEffettuate(int matricola){
        ...
        try {
            ...
            rs = stm.executeQuery("SELECT   tt.ref_dipendente, tt.ora_fine_turno, tt.ora_inizio_turno, ore_straordinarie_odierne " +
                    "FROM      turni tt " +
                    "WHERE    tt.ref_dipendente = " + matricola + " and tt.firma_ingresso = 1 and tt.data_odierna <= '" + LocalDate.now() + "' and tt.stato_giornaliero != 7 AND tt.stato_giornaliero != 4");  //ore che dobbiamo pagare
            while (rs.next()) {
    ...
    public ResultSet acquisisciRichiesteDip(int matricolaDip){
        try {
            ...
            rs = stm.executeQuery("SELECT cpv.ref_comunicazione, tpv.nome, cpv.data_inizio_pv, cpv.data_fine_pv, cpv.messaggio, c.ref_stato_comunicazione " +
                    "FROM tipo_proposta_variazione tpv, comunicazioni_proposta_variazione cpv, comunicazioni c, stati_comunicazione sc " +
                    "WHERE (cpv.ref_tipo_proposta_variazione = tpv.id_tipo_proposta_variazione) && (cpv.ref_comunicazione = c.id_comunicazione) " +
                    "&& (c.ref_stato_comunicazione = sc.id_stato_comunicazione) && (tpv.id_tipo_proposta_variazione != 8) && c.ref_utente_mittente = " + matricolaDip + " && (ref_utente_destinatario = " + matricolaDip + " || ref_utente_destinatario = " + db.acquisisciMatricolaAmm() + ") " +
                    "ORDER BY  cpv.ref_comunicazione " + "DESC; ");
            return rs;
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
