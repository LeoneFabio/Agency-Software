...
import static com.example.softwaredip.commons.Richiesta.TipoRichiesta.*;
import static com.example.softwaredip.commons.Utils.Tipo_messaggio.*;
import static com.example.softwaredip.commons.Utils.numeroGiorni;
import static com.example.softwaredip.commons.Utils.numeroOre;
...
public class AstensioneCtrl {
        ...
        if(db.creaConnessione()){
            ...
            if(dataDiInizio.isAfter(oggi) && isCompresoNelTrimestre(dataDiInizio, dataDiFine)){
                try {
                    ...
                    ResultSet rsTurno = db.acquisisciTurnoDip(Utente.utenteLoggato.getMatricola(), dataDiInizio);
                    ...
                    if(Utils.isNotEmptyResulset(rsTurno)){
                        ...
                        Utils.messaggioConsole("Nessun turno trovato", DEBUGGING);
                        rifiuta = true;
                    }
                    ...
                    ResultSet rsDip = db.acquisisciInfoUtente(Utente.utenteLoggato.getMatricola());
                    rsDip.next();
                ...
                */
                    ...
                    //INVIA RICHIESTA
                    int idComunicazione = db.insertComunicazionePV(Utente.utenteLoggato.getMatricola(), Utente.utenteLoggato.getMatricola(), oggi, dataDiInizio.atTime(oraInizio, 0), dataDiFine.atTime(oraFine, 0), tipoRichiesta.toString(), messaggio);
                    db.aggiornaStatoPV(idComunicazione, rifiuta ? 2 : 3);

                    //SE NON E RIFIUTATA
                    if(!rifiuta){
                        int[] matricoleIdonee = matricoleDipendentiIdonei(Utente.utenteLoggato.getMatricola(), oraInizio, dataDiInizio);

                        for(int dip : matricoleIdonee){
                            db.insertComunicazione(idComunicazione, Utente.utenteLoggato.getMatricola(), dip, oggi, "PV");
                            ...
                            //     int idComun = db.insertComunicazionePV(Utente.utenteLoggato.getMatricola(), dip, oggi, dataDiInizio.atTime(oraInizio, 0), dataDiFine.atTime(oraFine, 0), tipoRichiesta.toString(), messaggio);
                           // db.aggiornaStatoPV(idComun, 3);
                        }
                    }

                    Utils.messaggioConsole(
                            "Info richiesta: " + "\n" +
                                    "Mittente:" + Utente.utenteLoggato.getMatricola() + "\n" +
                                    "Destinatario:" + Utente.utenteLoggato.getMatricola() + "\n" +
                                    "Oggi:" + oggi + "\n" +
    ...
    private static void inviaFerie(Richiesta.TipoRichiesta tipoRichiesta, LocalDate dataDiInizio, LocalDate dataDiFine, int oraInizio, int oraFine, String messaggio){
        ...
        if(db.creaConnessione()){
            ...
            int ruolo;
            try {
                ResultSet rsGiorniPresi = db.acquisisciInfoUtente(Utente.utenteLoggato.getMatricola());
                rsGiorniPresi.next();
            ...
            if(dataDiInizio.isAfter(db.acquisisciDataFineTurnazione()) && (dataDiFine.getYear() == LocalDate.now().getYear()) && numeroGiorni(dataDiInizio, dataDiFine) <= sogliaFerie && LocalDate.now().isBefore(db.acquisisciDataFineTurnazione().minusDays(10))){
                 ...
                 */
                    ...
                    int idComunicazione = db.insertComunicazionePV(Utente.utenteLoggato.getMatricola(), matricolaAmministratore, oggi, dataDiInizio.atTime(0, 0), dataDiFine.atTime(0, 0), tipoRichiesta.toString(), messaggio);
                    db.aggiornaStatoPV(idComunicazione, rifiuta ? 2 : 3);
    ...
    private static void inviaMalattia(Richiesta.TipoRichiesta tipoRichiesta, LocalDate dataDiInizio, LocalDate dataDiFine, int oraInizio, int oraFine, String messaggio){
        ...
        if(db.creaConnessione()){
            ...
            if(dataDiInizio.isAfter(oggi) && isCompresoNelTrimestre(dataDiInizio, dataDiFine)){
                ...
                try{
                    ResultSet infoDip = db.acquisisciInfoUtente(Utente.utenteLoggato.getMatricola());
                    infoDip.next();
                ...
                //INVIA RICHIESTA
                int idComunicazione = db.insertComunicazionePV(Utente.utenteLoggato.getMatricola(), Utente.utenteLoggato.getMatricola(), oggi, dataDiInizio.atTime(0, 0), dataDiFine.atTime(0, 0), tipoRichiesta.toString(), messaggio);
                ...
                if(!rifiuta){
                    ...
                    while (dateIterator.isBefore(dataDiFine.plusDays(1))) {
                        int matricolaSostituto = calcoloDipendenteIdoneo(Utente.utenteLoggato.getMatricola(), dateIterator);

                        ResultSet rsTurno = db.acquisisciTurnoDip(Utente.utenteLoggato.getMatricola(), dateIterator);
                        ...
                        if(oraInizioTurno != -1 && oraFineTurno != -1){
                            ...
                            db.myUpdate("UPDATE `turni` SET `ora_inizio_turno`='-1',`ora_fine_turno`='-1',`stato_giornaliero`='" + 2 + "' WHERE data_odierna = '" + dateIterator + "' AND ref_dipendente = " + Utente.utenteLoggato.getMatricola());// metto il mittente a riposos e cambio il suo stato

                            db.creaNuovaComunicazioneCD(Utente.utenteLoggato.getMatricola(), matricolaSostituto, LocalDate.now(), "La informiamo che il suo turno di data " + dateIterator + " dalle ore " + oraInizioTurnoSostituto + " alle ore " + oraFineTurnoSostituto + " è stato sostituito con il turno dalle ore " + oraInizioTurno + " alle ore " + oraFineTurno + " del dipendente " + String.format("%04d", Utente.utenteLoggato.getMatricola()) + " (ruolo " + ruolo + ")");
    ...
    public static boolean verificaRichieste(LocalDate dataDiInizio, LocalDate dataDiFine) {
        ...
        if(db.creaConnessione()){
            ...
            try {
                ResultSet rsRichieste = db.acquisisciRichieste(Utente.utenteLoggato.getMatricola());
                ...
                if(Utils.isNotEmptyResulset(rsPIA)){
                    while (rsPIA.next()){
                        ...
                        if(!((dataDiInizio.isBefore(inizioPIA) && dataDiFine.isBefore(inizioPIA)) || (dataDiInizio.isAfter(finePIA) && dataDiFine.isAfter(finePIA)))){
                            ...
                            Utils.messaggioConsole("PIA già presente: dataInizio " + Utils.formattaData(inizioPIA) + ", dataFine:" + Utils.formattaData(finePIA), DEBUGGING);
                            rifiuta = true;
                        }
