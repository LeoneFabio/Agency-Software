...
import com.example.softwaredip.commons.EmailUtils;
...
public class AccountCtrl {
    ...
    public static void mostraRecuperaPsw() throws IOException {
    ...
    public static ResultSet acquisisciInfo(){
        ...
        ResultSet rs = db.acquisisciInfoUtente(Utente.utenteLoggato.getMatricola());
        try{
            rs.next();
    ...
    public static int[] prelevaAgevolazioni(){
        /*QUERY - acquisisciInfoDipendente(matricola) = info*/

        DBMSBound db = new DBMSBound();
        db.creaConnessione();
        return db.acquisisciAgevolazioni(Utente.utenteLoggato.getMatricola());
    }
    ...
    public static void salvaInformazioniDipendente(String email, String indirizzo, String cellulare){
        ...
        if(db.creaConnessione()){
            ...
            try {
                while(rsDip.next()){
                    int matricola = rsDip.getInt("matricola");
                    if(matricola != Utente.utenteLoggato.getMatricola() && rsDip.getString("cellulare").equals(cellulare)){
                        annulla = true;
                    }
                }
                rsInfoDip.next();
                if(!annulla && !cellulare.equals(rsInfoDip.getString("cellulare"))){
                    ...
                    Utils.mostraPopUp(Alert.AlertType.ERROR, "Errore dati", "Cellulare non valido");
                }
            } catch (SQLException e) {
                throw new RuntimeException(e);
            }
    ...
    private static void generaOTP(){
        ...
        String email = /*"mario.rossi.test52@gmail.com"*/ "nicoladinatali@gmail.com";
    ...
    public static void apriPaginaMenu(Stage stage, BorderPane borderPane, String title, String scenePath) throws IOException {
        ...
        if(scenePath.equals("gestioneturni/TurniWin.fxml") || scenePath.equals("gestionecomunicazioni/ComunRicevWin.fxml")){
